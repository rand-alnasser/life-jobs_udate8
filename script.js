...
function filterJobs() {
  ...
  var jobList = document.getElementsByClassName('job-list')[0];
  var jobs = jobList.getElementsByTagName('li');

  for (var i = 0; i < jobs.length; i++) {
    var job = jobs[i];
    var title = job.getElementsByTagName('h3')[0].innerText.toLowerCase();
    var location = job.getElementsByTagName('p')[0].innerText.toLowerCase();
    var salary = job.getElementsByTagName('p')[1].innerText.toLowerCase();

    if (title.includes(input) || location.includes(input) || salary.includes(input)) {
